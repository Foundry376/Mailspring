# Mailspring Discourse Reply Drafts — 2026-09-11

**Read this first — found something that touches part of this backlog.** `CHANGELOG.md` has listed **v1.24.0** as shipped 9/1/2026 for ten days now, but GitHub Releases still tops out at **1.23.0** (published 7/22) — no 1.24.0 tag, no installers. A community member noticed and asked directly: [#14568](https://community.getmailspring.com/t/mailspring-1-24-0-listed-in-changelog-but-no-release-packages-published/14568). I don't know whether the release step just got missed or this is intentional, so I've flagged it below for you rather than guessing — it's not something I can answer on your behalf.

This also affects three items already sitting in this backlog that said "update to v1.24.0" or "released 9/1" — **#22, #26, and #45** below. I've reworded those to say the fix is merged and queued for the next release, rather than implying there's a build users can actually download today, and changed #22 from Resolved to Reply since nothing has actually shipped yet. If 1.24.0 does exist somewhere and I'm just missing where it's published, let me know and I'll flip the wording back.

**Fifteenth consecutive week of a fully-researched draft, and `discourse-posted-ids.json` still hasn't changed once since this pipeline started.** This is now 57 items across fifteen weeks. I'm not going to re-argue the full case again this week — the complete version is in last week's file (`git show 904fb9d:discourse-replies-2026-09-04.md`). Short version, unchanged: either approve a batch — even just the oldest, most confident items — so posting actually starts happening, or tell me to pause or reset the pipeline so research time stops going toward replies that never reach anyone.

This week's actual new work, on top of carrying the existing backlog forward unchanged (aside from the three version-wording fixes above):
1. Confirmed the v1.24.0 release gap above. Related: the fix behind item #56 below (a Linux keyring failure) is an async key-manager rewrite you merged on 9/1 aimed at exactly this class of bug — but it's stuck in the same unpublished release, so it doesn't help that user yet.
2. Surveyed everything posted since 09-04 (six new threads). Three get replies below (**#55–57**); two are community plugin announcements with no question, skipped per the usual rule (**Junk Sender** and **Smart Folders**, both new this week); the sixth is the release-gap thread itself, flagged separately below.
...
> Good news — this is fixed. A community member ran into the same thing and sent in the fix ([PR #2821](https://github.com/foundry376/Mailspring/pull/2821)), which is merged and queued for the next release. I'll follow up here once it's actually out so you know when to update. Thanks for the original report, and thanks to the contributor who tracked it down and fixed it.
...
## New this week (2026-09-11)

### 55. Custom SMTP port input disappears when a typed prefix matches a preset (5877 stops at 587)
**Thread:** https://community.getmailspring.com/t/custom-smtp-port-input-disappears-when-a-typed-prefix-matches-a-preset-5877-stops-at-587/14565
**Action:** Reply

> Confirmed, and I can see exactly why. The custom port box only renders while your current value isn't one of the standard presets — the account setup form checks the field's numeric value against `[143, 993]` (IMAP) or `[25, 465, 587]` (SMTP) on every keystroke, and swaps the custom text input out for the preset dropdown the instant it matches. Typing `5877` one digit at a time passes through `587` as an intermediate value, which matches the SMTP preset exactly, so the form yanks the custom input away (taking focus with it) before you can type the final `7`. Two workarounds that avoid the bug entirely: paste the full `5877` instead of typing it digit-by-digit (a paste sets the whole value in one step, so it never passes through `587` on its own), or type a harmless extra digit first and delete it afterward (e.g. type `58770`, then backspace once) so the field is never exactly `587` mid-edit. I've flagged this for a real fix — the check needs to key off whether you're actively typing in the custom field, not just whether the current value happens to match a preset.

### 56. Fedora 44 KDE: Mailspring not properly interfacing with key storage after update
**Thread:** https://community.getmailspring.com/t/fedora-44-kde-mailspring-not-properly-interfacing-with-key-storage-after-update/14569
**Action:** Reply

> This is a known category of failure rather than something specific to your machine. Mailspring stores account passwords by asking Chromium (via Electron's `safeStorage`) to encrypt/decrypt a blob using whatever OS secret store is active — on KDE that's kwallet. When a system update rotates or resets the key kwallet uses internally (which a `kwalletd`/portal update can do), the blob Mailspring saved under the old key becomes permanently unreadable, and "decryption is not available" is Chromium reporting exactly that. On the release you're running, that failure is intentionally treated as fatal rather than silently continuing — resolving it to "no saved passwords" and carrying on could end up erasing every account's credentials the next time something gets written. I don't have a way to recover an already-unreadable blob from here; the practical fix is removing and re-adding the affected accounts so Mailspring saves fresh secrets under kwallet's current key. For more background on this class of error, see [this thread](https://community.getmailspring.com/t/password-management-error/199). Separately, worth knowing: we merged a hardening pass on 9/1 (an async rewrite of the key manager) specifically aimed at surviving this kind of Linux secret-service migration going forward — but it's sitting in a release that hasn't actually been published yet (see the note at the top of this file), so it won't help you today.

### 57. Spellcheck & grammar check in macOS doesn't work correctly
**Thread:** https://community.getmailspring.com/t/spellcheck-grammar-check-in-macos-doesnt-work-correctly/14564
**Action:** Reply

> Worth separating these into the two independent systems they actually are. Spellcheck is macOS's native dictionary-based checker (via Electron's session spellchecker) — it only flags words that aren't in the dictionary at all, so "Ho" and "God" won't be caught even though they're wrong in context, the same way no spellchecker flags "there" for "their." Grammar check is a separate, opt-in feature (**Preferences > Composing > "Check messages for grammar"**, off by default) that sends your text to LanguageTool for contextual analysis — and I found it explicitly filters out anything LanguageTool itself classifies as a plain misspelling, on the theory that's the spellchecker's job, not the grammar checker's. So a mistake LanguageTool happens to bucket internally as "misspelling" won't surface in Mailspring's grammar check either, even with the feature on — which would explain "no issues" on exactly this kind of typo. I can't confirm from here whether LanguageTool would have actually caught these specific examples if that filter weren't in place, since that's an external API call I can't run myself. Two things worth checking: is grammar check actually enabled in Preferences (it defaults to off), and could you share the literal text you typed? That would let us test it against the same API and confirm whether this is the categorization gap I found or something else entirely.

---

## Flagged for you separately — not included above, need your direct attention

- **Mailspring 1.24.0 listed in CHANGELOG but no release/packages published** (https://community.getmailspring.com/t/mailspring-1-24-0-listed-in-changelog-but-no-release-packages-published/14568) — new this week, and worth resolving before anything referencing v1.24.0 goes out from this batch. `CHANGELOG.md` has said 1.24.0 shipped 9/1 for ten days, but GitHub Releases still stops at 1.23.0 (7/22) — no tag, no installers. I don't know if the release step was missed or this is deliberate, so I haven't drafted a reply — this needs you to either publish it or tell the thread what's going on.
- **Calendar contribution + roadmap direction** (https://community.getmailspring.com/t/working-google-calendar-fork/14561 and the tail end of https://community.getmailspring.com/t/calendar-support/85) — still open, unchanged since last week. brhellman is asking directly whether a full CalDAV/O365/RSVP calendar implementation (built against 4 separate PRs) collides with work you already have planned, and whether to open it as a client PR or post it as one large commit. ejbiker93ss has a second, independent calendar fork with a different feature set (Outlook-style folder view, SmarterMail API, AI chat) and says plainly in thread #85: *"we don't know if the maintainer of the project even wants these features... we really need some guidance as to where this product is headed."* Two things I can confirm factually if you want a quick status reply before you weigh in on the bigger question: of the four PRs brhellman listed, **#2829** (typecheck fix) and the sync-engine hardening work (**Mailspring-Sync#122**) are merged into master and slated for v1.24.0, whenever that gets published (see the item above) — only **#2827** (mailsync startup error reporting, open, CI green, awaiting review) and the calendar engine/client work itself are still pending. The actual "should we accept this and what should the calendar roadmap be" question is entirely yours to answer — I don't have visibility into what you've already discussed with either contributor or planned internally.
...
- Two new community plugin announcements this week, no reply needed (announcements, not questions): **Junk Sender** (https://community.getmailspring.com/t/junk-sender-send-everything-from-a-sender-to-junk-with-one-click/14567) and **Smart Folders** (https://community.getmailspring.com/t/smart-folders-combine-folders-and-labels-across-accounts/14566).
