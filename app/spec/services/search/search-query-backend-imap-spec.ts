...
describe('IMAPSearchQueryBackend', () => {
  it('correctly codegens TEXT', () => {
    ...
    const result = IMAPSearchQueryBackend.compile(ast, { name: 'INBOX' } as any);
  ...
  it('correctly codegens AND', () => {
    ...
    const result1 = IMAPSearchQueryBackend.compile(ast1, { name: 'INBOX' } as any);
    expect(result1).toEqual(['FLAGGED', 'UNSEEN']);

    const ast2 = SearchQueryParser.parse('is:starred is:unread');
    const result2 = IMAPSearchQueryBackend.compile(ast2, { name: 'INBOX' } as any);
  ...
  it('correctly codegens "in:foo"', () => {
    ...
    const result2 = IMAPSearchQueryBackend.compile(ast1, { name: 'foo' } as any);
    expect(result2).toEqual([['OR', 'FLAGGED', 'ALL']]);
    const ast2 = SearchQueryParser.parse('in:foo');
    const result3 = IMAPSearchQueryBackend.compile(ast2, { name: 'foo' } as any);
    expect(result3).toEqual(['ALL']);
    const result4 = IMAPSearchQueryBackend.compile(ast2, { name: 'INBOX' } as any);
