# Mailspring Discourse Reply Drafts — 2026-08-07

**Read this note first:** This file replaces `discourse-replies-2026-07-24.md` and `discourse-replies-2026-07-31.md`, which I've deleted. Those two (and several before them going back to 2026-05-22) were drafted by past weekly runs but never actually reviewed or posted — `discourse-posted-ids.json` hasn't changed since the routine was first set up, and each week's run just kept re-drafting/consolidating on top of the last, un-reviewed, batch. That's a real process gap worth knowing about, separate from the content below. This week I re-verified everything against the current code myself rather than trust the carried-forward claims — one of them (that the composer supports pasted HTML tables) turned out to be **wrong**, which is exactly the risk of layering unreviewed drafts for 10 weeks. Full detail in my message to you.

Everything below is freshly researched this session, with file/line-level code verification where the finding is presented as confirmed. Where I wasn't confident, I said so and asked a clarifying question instead of guessing.
...
> I dug into this and have a solid lead, though I can't fully confirm the Yahoo-specific piece from the app's code alone. In 1.22.0 (released 6/13) we changed how a reminder gets attached to the sent thread — the composer now tags the draft with a `thread:`-prefixed marker and relies entirely on the sync engine to promote that to the plain marker the Reminders view actually looks for, once the message sends. If that promotion step doesn't complete for some reason, the reminder is silently stranded and the Reminders folder just stays empty with no error shown anywhere. The timing (your reminders stopped right around when 1.22.0 would've reached you) lines up well with this. Can you confirm you're on the current 1.23.0, and whether you've seen this on any non-Yahoo accounts too? That'll help tell us whether it's a general regression or Yahoo-specific. In the meantime, try **Preferences > Accounts > select the account > "Rebuild Cache"** — that forces a fresh sync handshake and may get a stuck reminder to promote correctly.
...
### 20. The page won't change when switch "Drafts" to "Activity" in sidebar
...
### 21. When I have 0 email in my inbox, there's a nice graphic showing up, but with some scroll bars glitch
...
### 22. E-mail row permanently green on mouse over
...
### 24. I keep getting crash reports
...
### 25. Problem with local email database
...
### 26. Attachements not visible in thread on Mac
...
### 27. Global menu not working when run under Wayland
...
### 29. Bug: New signature image overwrote old assets on Mailspring server (affecting old emails and shared accounts)
...
### 30. Thread (email) won't empty from trash
...
### 33. Could someone please review this post and let me know how to get v1.17 working on Fedora 43?
...
### 34. Still duplication sent on outlook
...
### 35. When I input Japanese, the sentence disappears during the input
...
### 36. Chinese Inputer not compatible
...
### 37. Select all - Limited to 200 messages?
...
- A handful of **Service Issues / Bugs** threads didn't have enough specific detail to draw a confident, evidence-based conclusion, so I left them unanswered rather than guess: **Mailspring will not authenticate shaw webmail** (#14452, VPN + wrong IMAP port), **Not able to install... SMTP Authentication Error 296** (#14243, Hostinger), **Gmail Won't Send** (#14496), **Encountered an error while syncing** (#14218), **Emails stuck on Monday, March 2nd** (#14276). Worth a look if you have a few minutes — some may just need a "please share your mailsync.log" follow-up.
